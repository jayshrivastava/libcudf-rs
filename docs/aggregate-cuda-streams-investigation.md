...
Investigation of the modest streams gains (~1.05-1.10x at 5M-20M rows) seen
in `aggregate-cuda-streams-bench-results.md`.
...
| Site | Why |
| --- | --- |
| `interop/to_arrow_host.cu:361,378,472,543` | every download syncs before returning host data |
| `groupby/hash/compute_aggregations.cuh:149` | every groupby reads a fallback `atomic_flag` |
| `detail/utilities/cuda_memcpy.hpp:87,103` | sync variant of memcpy helper |
| `detail/utilities/vector_factories.hpp` | many `make_*_vector_sync` helpers |
...
1. **Swap the device pool to `cuda_async_memory_resource`**
...
2. **Combine with pinned input.** This is the run we expect to actually move
   wall clock.
3. **NVTX-instrument** `flush_pending`, `from_arrow_host_on`,
   `to_arrow_host_on`, `groupby.aggregate_on` to attribute remaining syncs
   to specific operations.
4. **Production `pin_record_batch` in `CuDFLoadExec`** — staging copy with
...
once the upload serialization is gone.
